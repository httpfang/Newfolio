...
const skillsData: Record<SkillCategory, string[]> = {
  ...
  backend: ['Node.js', 'Express', 'Drizzle ORM', 'MongoDB', 'PostgreSQL'],
  tool: ['Git', 'GitHub', 'Vercel', 'Postman', 'Figma', 'Docker'],
};
    ...
    >
      ...
      <div className="container mx-auto max-w-7xl relative z-10">
        ...
        {isOverlayOpen && activeTab && (
          <div
            ref={overlayRef}
            className="fixed inset-0 z-[100] backdrop-blur-xl text-[#2D5016]"
            style={{
              background: 'linear-gradient(135deg, rgba(245, 243, 238, 0.98) 0%, rgba(232, 230, 225, 0.95) 100%)',
            }}
          >
            <div 
            ...
            <div className="relative flex flex-col h-full">
              <div className="flex items-center justify-between px-6 py-4 border-b border-[#2D5016]/10">
                <span className="text-xs tracking-widest uppercase font-serif text-[#2D5016]">
                  {PERSONAL_INFO.name || 'Portfolio'}
                </span>
                <button
                  aria-label="Close skills overlay"
                  className="close-btn text-3xl leading-none bg-transparent hover:bg-transparent font-serif text-[#2D5016]"
              ...
              <div className="flex-1 flex flex-col items-center justify-center px-6 py-12 md:py-20">
                <h3 className="overlay-title text-6xl md:text-8xl font-serif italic tracking-wide mb-12 md:mb-16 text-center text-[#2D5016]">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-6 md:gap-8 max-w-4xl w-full">
                  {skillsData[activeTab].map((skill) => (
                    <div
                      key={skill}
                      className="skill-item p-6 md:p-8 rounded-lg border border-[#2D5016]/10 bg-white/50 backdrop-blur-sm"
                      style={{
                        boxShadow: '0 2px 8px rgba(45, 80, 22, 0.04), 0 4px 16px rgba(45, 80, 22, 0.02)',
                      }}
                      onMouseEnter={(e) => {
                        gsap.to(e.currentTarget, {
                          scale: 1.03,
                          y: -4,
                          boxShadow: '0 8px 24px rgba(45, 80, 22, 0.08), 0 4px 12px rgba(45, 80, 22, 0.04)',
                          duration: 0.3,
                          ease: 'power2.out',
                          overwrite: 'auto',
                        });
                    ...
                    >
                      <span className="text-xl md:text-2xl font-medium text-[#2D5016] tracking-tight">
                        {skill}
                      </span>
                    </div>
                  ))}
